...
<meta property="og:site_name" content="collection.sooleg.com">
...
<div class="product-gallery">
	<img
		 ...
		 srcset="
				 ${imageUrls[0].replace('.jpg', '.jpg_100x100.jpg')} 100w,
				 ${imageUrls[0].replace('.jpg', '.jpg_324x324.jpg')} 324w,
				 ${imageUrls[0].replace('.jpg', '.jpg_500x500.jpg')} 500w
				 "
	...
	<div class="thumbnails">
		...
		<img 
			 ...
			 onclick="
					  ...
					  main.src = this.dataset.full.replace('.jpg', '.jpg_324x324.jpg');
...
position: 2,
name: safeFirst,
item: origin + "/nav?cat=" + encodeURIComponent(first)
});
}
...
name: safeSecond,
item: origin + "/nav?cat=" + encodeURIComponent(second)
});
}
...
export default {
	async fetch(request, env, ctx) {
		...
		if (!self.verificationList) {
			...
			const res = await fetch("https://collection.sooleg.com/url-sitemap.txt");
		...
		if (pathname === "/google2241541b2754940e.html") {
			const fileRes = await fetch("https://nde.buytostore.com/google2241541b2754940e.html");
...
<html lang="en">
	<head>
		<meta charset="UTF-8" />
		<meta name="viewport" content="width=device-width, initial-scale=1.0" />
		<title>collection.sooleg.com</title>
		<meta name="robots" content="noindex, follow" />
		<meta name="description" content="Find your dream product from thousands of deals. Shop smarter, live better with collection.sooleg.com." />
	...
	<body>
		<div class="logo" id="logo">collection.sooleg.com</div>
		...
		<script>
			window.addEventListener("DOMContentLoaded", () => {
				const logoText = "collection.sooleg.com";
...
`;
		...
		const staticExtensions = ['.ico', '.txt', '.txt.gz', '.xml', '.xml.gz', '.abeaseve'];
